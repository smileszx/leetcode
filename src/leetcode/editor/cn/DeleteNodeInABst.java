//450. 给定一个二叉搜索树的根节点 root 和一个值 key，删除二叉搜索树中的 key 对应的节点，并保证二叉搜索树的性质不变。返回二叉搜索树（有可能被更新）的
//根节点的引用。 
//
// 一般来说，删除节点可分为两个步骤： 
//
...
class Solution {
    public TreeNode deleteNode(TreeNode root, int key) {
        ...
        if (root.val == key && root.left != null) {
            ...
            deleteNode(root.left, root.left.val);
        }
        if (root.val == key && root.right != null) {
            deleteNode(root.right, root.right.val);
        }
        return root;
    }
}
